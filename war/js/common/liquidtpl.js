...
$.setFormValues = function( formId, jsonData, beforeSubmit ){
    ...
    $("#" + formId).find("input[type='checkbox']").each(function(){
        ...
        if(name.match(/\[\]$/)){
            var key = name.substr(0, name.length - 2);
            var valueList = targetValues[key];
...
$.template( "submitTemplate", '{{if id}}<input name="id" id="idField" type="hidden" value="${id}" \/>{{/if}}<input name="mode" type="hidden" value="submit" \/><input id="submit" type="submit" value="${submit}" \/>' );
