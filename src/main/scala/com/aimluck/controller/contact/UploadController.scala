    ...
    request.getParameter(Constants.KEY_MODE) match {
      case MODE_UPLOAD_URL =>{
          BasicHelper.writeJsonCommentFiltered(response, JsObject(List(
                ...
                (JsString(KEY_URL), tojson("/contact/upload?%s=%s".format(Constants.KEY_MODE, Constants.MODE_SUBMIT))),
      ...
      case Constants.MODE_SUBMIT => {
          if(UserDataService.isUserAdmin) {
            ...
            if((formFile != null) && (formFile.getData.size > 0)) {
              ...
              try {
                val contactCsv = ContactCsvService.upload(formFile, requestScope[String](KEY_CHARSET))
                if(!TextUtil.validateEmail(contactCsv.getSampleReplacers.apply(0))){
                ...
                BasicHelper.writeJsonCommentFiltered(response, JsObject(List(
                      ...
                      (JsString("sampleReplacers"), tojson(contactCsv.getSampleReplacers.toList)),
                      (JsString(Constants.KEY_ID), tojson(KeyFactory.keyToString(contactCsv.getKey)))
                    )))
              } catch {
